...
char question[1024]; //send this to all clients
char correctResponse[100] = "100"; //check client response with this string.
int rank = 1;
...
void *manager_child(void *ptr) {
	while (1) {
		...
		fgets(correctResponse, 1024, stdin);
		char *cptr = strchr(correctResponse, '\n');
		if (cptr)
			*cptr = '\0';
...
void *child(void *ptr) {
	...
	clientfds[nextClientIndex++] = connfd;

	char buffer[1024];
	//wait for response from client
	while (read(connfd, buffer, 1024)) {
		char *token = strtok(buffer, " \n");
		if (strcmp(token, correctResponse)) //mismatch?
