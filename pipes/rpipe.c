...
int main(int argc, char *argv[]) {
	int fds[2];
	char command1[MAXLEN], command2[MAXLEN];

	while (1) {
		printf("Enter command1: ");
		fgets(command1, MAXLEN, stdin);
		printf("Enter command2: ");
		fgets(command2, MAXLEN, stdin);
	...
	if (!fork()) {
		...
		//child: run command1?
		runCommand(command1);
	} else { //wait for both children to complete
		close(fds[0]); close(fds[1]);
		wait(NULL);
		wait(NULL);
	}
