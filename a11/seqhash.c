...
int main(int argc, char *argv[]) 
{
  ...
  //divide the file into n chunks and do hash individually
  int nChunks = atoi(argv[2]);
  int chunkSize = fileSize / nChunks; //assume perfect division!
  uint32_t hash = jenkins_one_at_a_time_hash(buffer, chunkSize);
  for(int i=1; i<nChunks; i++) {
	//hash ith chunk                               &buffer[i*chunkSize] is ok too!
  	uint32_t newHash = jenkins_one_at_a_time_hash(buffer + i*chunkSize, chunkSize);
	//combine existing hash & new hash values 
	//hash + newHash --> hash
	char combineBuffer[100];
	sprintf(combineBuffer, "%u%u", hash, newHash);
